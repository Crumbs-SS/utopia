...
public class AirplaneDAO extends BaseDAO implements ResultSetExtractor<List<Airplane>> {
    ...
    public void updateAirplane(Airplane airplane) {
        jdbcTemplate.update("UPDATE airplane SET type_id = ? WHERE id = ?", airplane.getAirplaneType().getId(), airplane.getId());
    }

    public void deleteAirplane(Airplane airplane) {
        jdbcTemplate.update("DELETE FROM airplane WHERE id = ?", airplane.getId());
    }
